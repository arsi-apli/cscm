...
/**
 * Implementation of the completion processing. The visual related processing is
 * done in AWT thread together with completion providers invocation and result
 * set sorting.
 * <br>
 * The only thing that can be done outside of the AWT is hiding of the
 * completion/documentation/tooltip.
 *
 * <p>
 * The completion providers typically reschedule computation intensive
 * collecting of their result set into an extra thread to keep the GUI
 * responsive.
 *
...
public class CompletionImpl extends MouseAdapter implements DocumentListener,
    ...
    /**
     * Document currently installed in the active component. Changed in AWT
     * only.
     */
    private WeakReference<Document> activeDocument = null;

    /**
     * Map containing keystrokes that should be overriden by completion
     * processing. Changed in AWT only.
     */
    private InputMap inputMap;

    /**
     * Action map containing actions bound to keys through input map. Changed in
     * AWT only.
     */
     ...
     * <br>
     * Initiated in AWT and can be cleared from the thread that cancels the
     * completion query.
     */
    ...
    /**
     * Timer for opening documentation window automatically. Changed in AWT
     * only.
     */
    ...
    private CompletionProvider[] getCompletionProvidersForComponent(JTextComponent component, boolean asyncWarmUp) {
        ...
        Collection<? extends GlobalCompletionProvider> global = Lookup.getDefault().lookupAll(GlobalCompletionProvider.class);
        List<CompletionProvider> sum = new ArrayList<>(col);
        sum.addAll(global);
        int size = sum.size();
        CompletionProvider[] ret = size == 0 ? null : sum.toArray(new CompletionProvider[size]);
    ...
    /**
     * Called from caretUpdate() to refresh the completion result after caret
     * move.
     * <br>
    ...
    /**
     * Called from dispatchKeyEvent() to insert prefix common to all items in
     * the completion result after TAB.<br>
     * Must be called in AWT thread after all tasks of the current
     * completionResult are finished.
     */
    ...
    /**
     * Request displaying of the completion pane. Can be called from any thread
     * - is called synchronously from the thread that finished last unfinished
     * result.
     */
    ...
    /**
     * May be called from any thread. The UI changes will be rescheduled into
     * AWT.
     */
    ...
    /**
     * Request displaying of the documentation pane. Can be called from any
     * thread - is called synchronously from the thread that finished last
     * unfinished result.
     */
    ...
    /**
     * Request displaying of the tooltip pane. Can be called from any thread -
     * is called synchronously from the thread that finished last unfinished
     * result.
     */
    ...
    /**
     * Find first result that has non-null documentation or tooltip depending on
     * its query type.
     * <br>
     ...
     * @return Distance between the strings. (Number of changes which have to be
     * done to get from <CODE>s</CODE> to <CODE>t</CODE>.
     */
     ...
     * <br>
     * If the result is finished then cancelling physically cancels the result
     * sets.
     */
    final class Result {
         ...
         * <br>
         * If the result is not prepared a flag that the result was cancelled is
         * turned on (and later returned from prepared()).
         * <br>
        ...
        /**
         * and return the new result set containing the refreshed results.
         */
        ...
        /**
         * Invoke refreshing of the result sets. This method should be invoked
         * on the result set returned from {@link #createRefreshResult()}.
         */
