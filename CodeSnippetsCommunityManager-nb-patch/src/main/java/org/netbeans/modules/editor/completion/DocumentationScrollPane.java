...
public class DocumentationScrollPane extends JScrollPane {
    ...
    private static final String RANK = "org/netbeans/modules/editor/completion/resources/rank.png";
    private static final String RANK_OFF = "org/netbeans/modules/editor/completion/resources/rank_off.png";
    ...
    public void setData(CompletionDocumentation doc) {
        ...
        if ((doc instanceof RankProvider) && !((RankProvider) doc).isLocal()) {
            rankView.setVisible(true);
            rankEdit.setVisible(true);
            rankView.setRank(((RankProvider) doc).getRank());
            rankEdit.setProvider((RankProvider) doc);

        } else {
            rankView.setVisible(false);
            rankEdit.setVisible(false);
            rankEdit.setProvider(null);
        }
    }
    ...
    private void installTitleComponent() {
        ...
        gdc.weightx = 1.0;
        gdc.anchor = GridBagConstraints.WEST;
        toolbar.add(rankView, gdc);

        rankEdit = new RankEdit();
        gdc.gridx = 10;
        gdc.gridy = 0;
        gdc.weightx = 1.0;
        gdc.anchor = GridBagConstraints.WEST;
        toolbar.add(rankEdit, gdc);
